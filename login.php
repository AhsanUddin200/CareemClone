...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email && $password) {
        $stmt = $pdo->prepare("SELECT id, password FROM careem_users WHERE email = ?");
        $stmt->execute([$email]);
...
<style>
    ...
    h1 {
        ...
        margin-bottom: 20px;
        font-size: 1.8rem;
        color: #28a745; /* Careem green color */
    }
    ...
    button {
        width: 100%;
        padding: 12px;
        background: #28a745; /* Careem green color */
        border: none;
        border-radius: 8px;
        color: #fff;
        font-size: 1rem;
